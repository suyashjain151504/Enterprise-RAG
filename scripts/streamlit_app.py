...
def _eval_dashboard_section() -> None:
    ...
    m1.metric("Faithfulness", f"{(agg.get('faithfulness') or 0):.2f}")
    m2.metric("Ctx Precision", f"{(agg.get('context_precision') or 0):.2f}")
    m3.metric("Ctx Recall", f"{(agg.get('context_recall') or 0):.2f}")
    m4.metric("Ans Relevancy", f"{(agg.get('answer_relevancy') or 0):.2f}")
    ...
    if prev_data:
        ...
        st.caption(
            f"vs previous: "
            f"faith {_delta(agg.get('faithfulness') or 0, prev_agg.get('faithfulness') or 0)} · "
            f"prec {_delta(agg.get('context_precision') or 0, prev_agg.get('context_precision') or 0)} · "
            f"recall {_delta(agg.get('context_recall') or 0, prev_agg.get('context_recall') or 0)} · "
            f"ans_rel {_delta(agg.get('answer_relevancy') or 0, prev_agg.get('answer_relevancy') or 0)}"
        )
